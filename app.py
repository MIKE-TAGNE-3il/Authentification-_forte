...
def select_auth_method():
    ...
    if selected_method == "nfc":
        return redirect(url_for("nfc_auth_setup"))
...
@app.route("/nfc-auth/setup")
def nfc_auth_setup():
    if "user_id" not in session:
        return redirect(url_for("login"))
    # réinitialise l'état de vérification NFC pour cette session
...
def read_nfc():
    ...
    if readers is None:
        return jsonify({"error": "pyscard (smartcard) not installed"}), 500

    try:
        r = readers()
        if not r:
            return jsonify({"error": "no reader"}), 500
        ...
        return jsonify({"error": "lecture failed", "sw1": sw1, "sw2": sw2}), 500
    except Exception as exc:
        return jsonify({"error": str(exc)}), 500


@app.route("/nfc-auth/report")
def nfc_auth_report():
    if "user_id" not in session:
        return redirect(url_for("login"))
