"""
Helpers TOTP compatibles Microsoft Authenticator.
...
Il n'a pas de dependance externe.
"""
...
def generate_totp_secret(num_bytes: int = 20) -> str:
    """
    Genere un secret aleatoire base32 (RFC 6238).
    20 bytes (~160 bits) est une valeur standard.
    """
...
def build_otpauth_uri(secret: str, account_name: str, issuer: str) -> str:
    """
    Construit une URI otpauth scanable par Microsoft Authenticator.
    """
...
def build_qr_code_url(otpauth_uri: str, size: str = "250x250") -> str:
    """
    Retourne une URL d'image QR (service public quickchart).
...
def verify_totp(
    ...
    """
    Verifie un code TOTP utilisateur.

    valid_window=1 accepte la fenetre precedente, courante et suivante
    (tolere un leger decalage d'horloge).
    """
